...
技术栈：**阿里云通义千问 Qwen（大模型）+ LangGraph（编排）+ FAISS（向量库）+ bge 中文嵌入 + Streamlit（网页）**。
...
| .env | 存阿里云 DashScope Key（已被 gitignore，绝不外传） | 填你自己的 Key |
| start.bat | 双击即用项目 venv 启动 | 懒人用 |

## 二、本地运行（3 选 1）

**前提**：`.env` 里已写好 `DASHSCOPE_API_KEY=sk-你的key`（程序自动读取，不用每次手填）。
...
- **生成**：通义千问（Qwen）基于检索到的文档片段作答，避免凭空编造（幻觉）。
...
3. **Streamlit Cloud 部署**：打开 https://share.streamlit.io → 用 GitHub 登录 → New app → 选刚才的仓库、主文件填 `app.py` → 展开 Advanced settings：
   - **Secrets** 填：`DASHSCOPE_API_KEY = "你的key"`
